...
<div id="desktop-root" data-desktop-app-root data-decoration="<?= p(in_array(($_['decoration'] ?? 'standard'), ['redmond', 'retro'], true) ? $_['decoration'] : 'standard') ?>" data-decoration-color="<?= p($_['decorationColor'] ?? 'nextcloud') ?>" data-icon-decoration="<?= p($_['iconDecoration'] ?? 'standard') ?>" data-icon-decoration-linked="<?= !empty($_['iconDecorationLinked']) ? 'true' : 'false' ?>" data-icon-color="<?= p($_['iconColor'] ?? 'nextcloud') ?>" data-window-controls-side="<?= ($_['windowControlsSide'] ?? 'right') === 'left' ? 'left' : 'right' ?>" data-shell-mode="<?= ($_['shellMode'] ?? 'taskbar') === 'dock' ? 'dock' : 'taskbar' ?>" data-dock-always-visible="<?= !isset($_['dockAlwaysVisible']) || $_['dockAlwaysVisible'] ? 'true' : 'false' ?>" data-desktopfiles-enabled="<?= $desktopfilesEnabled ? 'true' : 'false' ?>" data-show-favorites="<?= !empty($_['showFavorites']) ? 'true' : 'false' ?>" data-favorites-no-confirm="<?= !empty($_['favoritesNoConfirm']) ? 'true' : 'false' ?>" data-show-trash="<?= !empty($_['showTrash']) ? 'true' : 'false' ?>" data-show-home="<?= !empty($_['showHome']) ? 'true' : 'false' ?>" data-desktop-folder="<?= p($_['desktopFolder'] ?? '') ?>" data-trash-no-confirm="<?= !empty($_['trashNoConfirm']) ? 'true' : 'false' ?>" data-personal-save-url="<?= p($_['personalSaveUrl'] ?? '') ?>" data-heartbeat-url="<?= p($_['heartbeatUrl'] ?? '') ?>" data-dynamic-data-url="<?= p($_['dynamicDataUrl'] ?? '') ?>" data-icon-positions="<?= p($_['iconPositions'] ?? '{}') ?>" data-icon-save-url="<?= p($_['iconSaveUrl'] ?? '') ?>" data-app-pins="<?= p(($_['appPins'] ?? '') !== '' ? $_['appPins'] : '{"taskbar":[],"desktop":[]}') ?>" data-app-pins-configured="<?= ($_['appPins'] ?? '') !== '' ? 'true' : 'false' ?>" data-app-pins-save-url="<?= p($_['appPinsSaveUrl'] ?? '') ?>" data-apps-menu-size="<?= p(($_['appsMenuSize'] ?? '') !== '' ? $_['appsMenuSize'] : '{}') ?>" data-apps-menu-size-configured="<?= ($_['appsMenuSize'] ?? '') !== '' ? 'true' : 'false' ?>" data-apps-menu-size-save-url="<?= p($_['appsMenuSizeSaveUrl'] ?? '') ?>" data-browser-state-migrated="<?= !empty($_['browserStateMigrated']) ? 'true' : 'false' ?>" data-browser-state-migration-url="<?= p($_['browserStateMigrationUrl'] ?? '') ?>" data-window-states="<?= p($_['windowStates'] ?? '{"windows":[]}') ?>" data-window-save-url="<?= p($_['windowSaveUrl'] ?? '') ?>" data-first-visit="<?= !empty($_['firstVisit']) ? 'true' : 'false' ?>" data-apps="<?= p(json_encode($apps, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT)) ?>">
    ...
    <div id="desktop-topbar" class="desktop-topbar">
        <div id="desktop-shell-utilities" class="desktop-shell-utilities">
            ...
            <time id="desktop-clock" class="desktop-clock"><span class="desktop-clock-face" aria-hidden="true"><i class="desktop-clock-hour"></i><i class="desktop-clock-minute"></i><i class="desktop-clock-second"></i><i class="desktop-clock-pin"></i></span></time>
